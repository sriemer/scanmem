...
class GameConqueror():
    ...
    def update_scan_result(self):
        ...
        if (match_count > SCAN_RESULT_LIST_LIMIT) or (self.backend.check_process(self.pid) != PROC_RUNNING):
            self.scanresult_liststore.clear()
        else:
            self.command_lock.acquire()
    ...
    def data_worker(self):
        if (self.is_scanning) or (self.pid == 0) or (self.backend.check_process(self.pid) != PROC_RUNNING):
